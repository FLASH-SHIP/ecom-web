...
// ── Component ─────────────────────────────────────────────────────────────────
export function PhoneInput({ value = "", onChange, label, error, disabled, id = "phone-input", placeholder = "Enter a phone number", }) {
    ...
    const filteredCountries = React.useMemo(() => {
        ...
        return baseList.filter((c) => c.name.toLowerCase().includes(q) || c.dial.includes(q) || c.code.toLowerCase().includes(q));
    }, [search]);
    ...
    return (_jsxs("div", { className: "flex flex-col gap-1.5 w-full", children: [label && (_jsx("label", { htmlFor: id, className: cn("text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70", error && "text-destructive"), children: label })), _jsxs("div", { className: "flex w-full", children: [enableCountryCode && (_jsxs(Popover, { modal: true, open: open, onOpenChange: (o) => {
                            setOpen(o);
                            if (!o)
                                setSearch("");
                        }, children: [_jsx(PopoverTrigger, { asChild: true, children: _jsxs(Button, { type: "button", variant: "outline", disabled: disabled, className: cn("flex h-10 items-center gap-1 rounded-r-none border-r-0 px-3 focus:z-10 bg-background hover:bg-accent", error && "border-destructive"), "aria-label": "Select country code", children: [_jsx("span", { className: "text-base leading-none", children: selectedCountry.flag }), _jsx(ChevronsUpDown, { className: "-mr-1 size-3.5 shrink-0 opacity-60" })] }) }), _jsxs(PopoverContent, { className: "flex w-[300px] flex-col overflow-hidden p-0 bg-popover", align: "start", children: [_jsxs("div", { className: "flex shrink-0 items-center border-b border-border px-3 py-2", children: [_jsx(Search, { className: "mr-2 size-4 shrink-0 text-muted-foreground" }), _jsx("input", { value: search, onChange: (e) => setSearch(e.target.value), placeholder: "Search country...", className: "flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground text-foreground" })] }), _jsx("div", { className: "h-72 overflow-y-auto p-1", children: filteredCountries.length === 0 ? (_jsx("div", { className: "py-6 text-center text-sm text-muted-foreground", children: "No country found." })) : (filteredCountries.map((country) => (_jsxs("button", { type: "button", onClick: () => handleCountrySelect(country), className: "flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-sm hover:bg-accent hover:text-accent-foreground text-foreground text-left", children: [_jsx("span", { className: "text-base", children: country.flag }), _jsx("span", { className: "flex-1 truncate", children: country.name }), _jsx("span", { className: "text-xs text-muted-foreground", children: country.dial }), _jsx(Check, { className: cn("size-3.5 shrink-0", country.code === selectedCountry.code ? "opacity-100" : "opacity-0") })] }, country.code)))) })] })] })), _jsx(Input, { id: id, type: "tel", value: number, onChange: handleNumberChange, disabled: disabled, placeholder: placeholder, "aria-invalid": !!error, minLength: minLength, maxLength: inputMaxLength, className: cn(enableCountryCode && "rounded-l-none", error && "border-destructive") })] }), error && _jsx("p", { className: "text-xs text-destructive", children: error })] }));
}
