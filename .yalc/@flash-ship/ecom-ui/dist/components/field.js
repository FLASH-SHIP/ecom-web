...
function FieldGroup({ className, ...props }) {
    return (_jsx("div", { "data-slot": "field-group", className: cn("group/field-group @container/field-group flex w-full flex-col gap-4 data-[slot=checkbox-group]:gap-3 [&>[data-slot=field-group]]:gap-4", className), ...props }));
}
...
function Field({ className, orientation = "vertical", ...props }) {
    ...
    _jsx("div", { role: "group", "data-slot": "field", "data-orientation": orientation, className: cn(fieldVariants({ orientation }), className), ...props }));
}
...
function FieldLabel({ className, ...props }) {
    return (_jsx(Label, { "data-slot": "field-label", className: cn("text-xs lg:text-sm 2xl:text-base text-[#232323] font-medium", "group/field-label peer/field-label flex w-fit gap-1 leading-snug group-data-[disabled=true]/field:opacity-50", "has-[>[data-slot=field]]:w-full has-[>[data-slot=field]]:flex-col has-[>[data-slot=field]]:rounded-md has-[>[data-slot=field]]:border [&>*]:data-[slot=field]:p-4", "has-data-[state=checked]:border-primary has-data-[state=checked]:bg-primary/5 dark:has-data-[state=checked]:bg-primary/10", className), ...props }));
}
...
function FieldError({ className, children, errors, ...props }) {
    const content = useMemo(() => {
        ...
        return (_jsx("ul", { className: "ml-4 flex list-disc flex-col gap-1", children: uniqueErrors.map((error, index) => error?.message ? (
            // biome-ignore lint/suspicious/noArrayIndexKey: error messages are static and unique within uniqueErrors map
            _jsx("li", { children: error.message }, index)) : null) }));
    }, [children, errors]);
    if (!content) {
        return null;
    }
